...
userRouter.post("/" , ( req : Request , res : Response , next) => {
    ...
    if(isExit) {
        return res.status(200).json({ user : isExit , admin , roomCategories , roomImages , extraImages , musics })
    } else {
        const newUser = await prisma.user.create({ data : { email , name , url } })
        res.status(200).json({ user : newUser , admin , roomCategories , roomImages , extraImages , musics})
    }
})
