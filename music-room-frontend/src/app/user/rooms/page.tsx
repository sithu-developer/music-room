...
const RoomsPage = () => {
    ...
    return (
        <Box sx={{ height : "100vh" ,  display : "flex" , flexDirection : "column" , alignItems : "center" , bgcolor : "primary.light" }} >
            <Box sx={{ display : "flex" , justifyContent : "space-between" , alignItems : "center" , width : "100%" , px : "20px" , py : "8px" , bgcolor : "primary.main" }} >
                <UserSideBar />
                <Typography variant="h4" sx={{ ml : "60px"}} >Rooms</Typography>
                <IconButton onClick={() => setOpenNewRoom(true)} >
                    <ChairRoundedIcon sx={{ color : "whitesmoke" , fontSize : "30px"}} />
                </IconButton>
            </Box>
            <Box sx={{ display : "flex" , gap : "20px" , width : "100vw" , p : "0 30px 10px 30px" , overflowX : "auto" , ml : "90px" }} >
                {categories.map(item => (
