...
const NewMusicDialog = ({ open , setOpen } : Prop ) => {
    const [ newMusic , setNewMusic ] = useState<DefaultNewMusicType>({ name : "" , artist : "" });
    ...
    const onMusicImgDrop = useCallback((backgroundImages : File[]) => {
        ...
        setNewMusic(prev => ({...prev , muiscImg : musicImage}) )
    }, [])

    const musicImgDropItems = useDropzone({onDrop : onMusicImgDrop })
    ...
    return (
        <Dialog open={open} onClose={() => {
            ...
            <DialogContent sx={{ display : "flex" , flexDirection : "column" , gap : "20px"  , bgcolor : "#5182e3" , border : "1px solid #3e648c"}}>
                ...
                <TextField label="Artist (Optional)" color="secondary" sx={{ input : { color : "white" }}} onChange={(e) => setNewMusic({...newMusic , artist : e.target.value}) }  />
                ...
                <Box sx={{ display : "flex" , flexDirection :"column" , gap : "5px" , alignItems : "start" }}>
                    ...
                    {newMusic.muiscImg && <Chip color="primary" sx={{ maxWidth : "250px"}} label={newMusic.muiscImg.name} onDelete={() => setNewMusic(prev => ({...prev , muiscImg : undefined}))} />}
                </Box>
                <Box sx={{ display : "flex" , flexDirection :"column" , gap : "5px" , alignItems : "start" }}>
